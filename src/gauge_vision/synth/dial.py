...
class DialTruth:
    ...
    `angle_deg` İP6'nın (Hough ibre açısı) hedefi, `value` İP7'nin (açı→değer)
    hedefi, `bbox` ise İP5'in (YOLO tespiti) hedefi.
    """
...
def render_analog(
    ...
    *,
    ...
    radius = int(size * FACE_RADIUS_RATIO)
    ...
    tip = _draw_needle(img, center, radius, angle, needle_bgr)
...
def _draw_needle(img, center, radius, angle_deg, needle_bgr) -> tuple[int, int]:
    ...
    width = max(1, round(radius * NEEDLE_W_RATIO))
